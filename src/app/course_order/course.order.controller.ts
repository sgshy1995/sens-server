...
export class CourseOrderController {
  ...
  async findOneCourseOrderByOrderNo(@Param("order_no") order_no: string, @Res({ passthrough: true }) response: Response, @Req() request: RequestParams): Promise<Response | void | Record<string, any>> {
    const res = await this.courseOrderService.findOneCourseOrderByOrderNo(order_no);
    response.status(res.code);
    return res;
  }
  ...
  @UseGuards(AuthGuard("jwt")) // 使用 'JWT' 进行验证
  @Get("id/:id")
  async findOneCourseOrderById(@Param("id") id: string, @Res({ passthrough: true }) response: Response, @Req() request: RequestParams): Promise<Response | void | Record<string, any>> {
    const res = await this.courseOrderService.findOneCourseOrderById(id);
    response.status(res.code);
    return res;
  }
